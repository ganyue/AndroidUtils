...
public class A2dpConnecter {

    private static final String TAG = A2dpConnecter.class.getSimpleName();
    ...
    private OnA2dpConnectorListener onA2dpConnectorListener;
    public A2dpConnecter (final Context context, OnA2dpConnectorListener listener) {
        this(context);
        this.onA2dpConnectorListener = listener;
    }

    public A2dpConnecter(final Context context) {
        mContext = context;
        onInit();
    }
    ...
    private void doBTConnect() throws Exception{
        ...
        if (isDevicePaired(mBluetoothAdapter.getBondedDevices())) {
            if (isA2dpConnected()) {
                if (onA2dpConnectorListener != null) onA2dpConnectorListener.onA2dpConnectSuccess();
            } else {
    ...
    private void connectToA2dp(final BluetoothDevice device) throws Exception{
        ...
        if(isSuccess){
            ...
            if (onA2dpConnectorListener != null) onA2dpConnectorListener.onA2dpConnectFail();
        }
    }
    ...
    public interface OnA2dpConnectorListener {
